...
export class ConfluenceUtil {
  ...
  async listSpaces(profileName: string, format: 'json' | 'toon' = 'json'): Promise<ApiResult> {
    try {
      ...
      const simplifiedSpaces = spaces.map((s: { key?: string; name?: string; type?: string; id?: number }) => ({
        key: s.key,
        name: s.name,
        type: s.type,
        id: String(s.id),
  ...
  async listPages(
    ...
    try {
      ...
      const results = (
        response as {
          results?: Array<{ id: string; title: string; type: string; status: string; space?: { key: string } }>;
        }
      ).results;
  ...
  async addComment(
    ...
    try {
      ...
      const spaceKey = (page as { space?: { key?: string } }).space?.key;
